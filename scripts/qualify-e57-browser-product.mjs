...
function outputArgument(values) {
  ...
  if (
    ...
    throw new TypeError(
      "usage: node scripts/qualify-e57-browser-product.mjs " +
        "[--out path]",
    );
  }
...
function exactSurface(surface) {
  return (
    ...
    surface.fixture.sourceBytes === 118_784 &&
    surface.fixture.fingerprint ===
      "sha256:6dbf7972b358bd7dd0864c7893a4aa7b" +
        "61a339fd6ee27c71b3031f763c977d33" &&
    surface.fixture.formatVersion === "1.0" &&
    surface.fixture.pointFormat === "cartesian-xyz-rgb" &&
    ...
    surface.fixture.provenance.license === "CC0-1.0" &&
    surface.fixture.provenance.bundled === false &&
    surface.fixture.provenance.sampleRedistributed === false &&
    exactPointModel(surface.observation?.model, 7_680, 1, 1) &&
    surface.observation.resources?.sourceBytes === 118_784 &&
    surface.observation.resources.decodedPointBytes === 215_040 &&
    surface.observation.resources.pointRangeBytes === 122_928 &&
    ...
    surface.observation.renderer.sourceReadBytes === 122_928 &&
    surface.observation.renderer.uploadedBytes === 122_880 &&
    same(surface.observation.pointCloud?.bounds, {
      min: [-0.5, -0.5, -0.5],
      max: [0.5, 0.5, 0.5],
    }) &&
    ...
    surface.observation.pointCloud.coordinateReferenceStatus ===
      "unqualified" &&
    surface.observation.pointCloud.maximumProjectionError === 0 &&
...
export function validateE57BrowserProductQualification(report) {
  if (
    report?.schema !==
      "bim-explorer-e57-browser-product-evidence/1" ||
    report.status !==
    ...
    report.decision.renderer !== "passed-browser-webgl2" ||
    ...
    throw new Error(
      "E57 Browser product qualification evidence is invalid",
    );
  }
  return report;
}

export async function qualifyE57BrowserProduct() {
  const surface = await qualifyBimProductShell({
    fixture: "e57-public",
  });
  const report = {
    schema: "bim-explorer-e57-browser-product-evidence/1",
  ...
  return validateE57BrowserProductQualification(report);
}

async function main() {
  const output = outputArgument(process.argv.slice(2));
  const report = await qualifyE57BrowserProduct();
  if (output !== null) {
