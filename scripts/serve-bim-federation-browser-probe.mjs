...
export function createBimFederationBrowserProbeServer({
  input,
  ranges,
}) {
  if (
    ...
    throw new TypeError(
      "federation Browser probe input is invalid",
    );
  }
  const layer = input.snapshot.layers.find((candidate) =>
  ...
  if (
    ...
    throw new Error("federation Browser ranges are invalid");
  }
