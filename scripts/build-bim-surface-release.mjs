...
import {
  ...
  stageBimSurfacePackage,
} from "./qualify-bim-surface-package.mjs";
...
const RELEASE_DOCUMENTS = Object.freeze([
  ...
  ["packages/bim-surface/SOURCE_OFFER.md", "SOURCE_OFFER.md"],
  [
    "docs/releases/bim-surface-v0.1.0.md",
    "RELEASE_NOTES.md",
  ],
]);
...
async function requireEmptyOutput(outputDirectory) {
  ...
  if (
    ...
    throw new TypeError("BIM surface release output is unsafe");
  }
  try {
    const metadata = await stat(resolved);
    if (!metadata.isDirectory()) {
      throw new TypeError("BIM surface release output must be a directory");
    }
    if ((await readdir(resolved)).length > 0) {
      throw new Error(
        "BIM surface release output directory must be empty",
      );
    }
  } catch (error) {
...
function requireCleanSource() {
  ...
  if (status.length > 0) {
    throw new Error(
      "BIM surface release requires a clean tracked source tree",
    );
  }
}
...
function createSpdx({
  ...
  const { name, version, releaseTag } = BIM_SURFACE_PACKAGE;
  return Object.freeze({
    spdxVersion: "SPDX-2.3",
    dataLicense: "CC0-1.0",
    SPDXID: "SPDXRef-DOCUMENT",
    name: `bim-explorer-bim-surface-${version}`,
    documentNamespace:
      `https://spdx.org/spdxdocs/` +
      `bim-explorer-bim-surface-${version}-${commit}`,
    creationInfo: {
      created: createdAt,
      creators: [
        "Tool: bim-explorer-build-bim-surface-release/1",
      ],
    },
    packages: [
      {
        name,
        SPDXID: "SPDXRef-Package-BimSurface",
        versionInfo: version,
        downloadLocation:
          `https://github.com/menaje/bim-explorer/releases/` +
          `download/${releaseTag}/${packageArtifact.filename}`,
        filesAnalyzed: false,
        checksums: [
          {
            algorithm: "SHA256",
    ...
    relationships: [
      {
        ...
        relatedSpdxElement: "SPDXRef-Package-BimSurface",
      },
    ],
  });
}

export async function buildBimSurfaceRelease(outputDirectory) {
  requireCleanSource();
  const output = await requireEmptyOutput(outputDirectory);
  const qualification = await qualifyBimSurfacePackage();
  const temporary = await mkdtemp(
    path.join(tmpdir(), "bim-surface-release-"),
  );
  try {
    const stage = path.join(temporary, "stage");
    await stageBimSurfacePackage(stage);
    const packageArtifact = await packBimSurfacePackage(
      stage,
    ...
    if (
      ...
      throw new Error(
        "BIM surface release package differs from qualification",
      );
    }
    ...
    const sbomFile =
      ...
      `${BIM_SURFACE_PACKAGE.version}.spdx.json`;
    await writeFile(
    ...
    const releaseManifest = {
      schema: "bim-explorer-bim-surface-release/1",
      package: {
        name: BIM_SURFACE_PACKAGE.name,
        version: BIM_SURFACE_PACKAGE.version,
        contract: BIM_SURFACE_PACKAGE.contract,
        license: "MPL-2.0",
      ...
      source: {
        ...
        commit,
        expectedTag: BIM_SURFACE_PACKAGE.releaseTag,
        createdAt,
        cleanTree: true,
      },
      profile: {
    ...
    return Object.freeze({
      output,
      version: BIM_SURFACE_PACKAGE.version,
      tag: BIM_SURFACE_PACKAGE.releaseTag,
      commit,
...
function parseArguments(values) {
  if (
    ...
    throw new TypeError(
      "usage: node scripts/build-bim-surface-release.mjs " +
        "--out <directory>",
    );
  }
...
if (
  ...
  const result = await buildBimSurfaceRelease(
    parseArguments(process.argv.slice(2)),
  );
  process.stdout.write(
    `BIM Surface ${result.version} release: ` +
