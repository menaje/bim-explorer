...
`.bin`/`.png`를 합산 64MiB 안에서 받습니다. 외부 PNG, exact glTF PNG data URI
또는 GLB PNG bufferView texture는 저장 방식 전체에 encoded 8MiB, decoded RGBA
16MiB, 축당 2,048px와 256:1 ratio를 추가 적용합니다.
...
- arbitrary glTF URI, glTF bufferView image, JPEG·투명/다중 material texture,
  Draco와 승인되지 않은 required extension; bounded same-folder `.bin`/`.png`,
  외부/data-URI/GLB-bufferView OPAQUE PNG `baseColorTexture`,
