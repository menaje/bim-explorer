...
`.bin`/`.png`, 합산 64MiB입니다. OPAQUE PNG `baseColorTexture`는 외부 PNG,
exact glTF PNG data URI와 GLB PNG bufferView에서 geometry-range v2로
투영합니다. exact BoxTextured 외부 bundle과 embedded GLB는 각각 같은 세 Apple
...
통과했습니다. 임의 URI·glTF bufferView image·JPEG·비-OPAQUE alpha material
mode·다른 material texture role·Draco·다른 meshopt
